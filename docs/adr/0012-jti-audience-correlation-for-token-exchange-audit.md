...
[ADR 0005](0005-delegation-audit-with-opentelemetry.md)で導入した`audit/audit.py`のCHECK2は、downstreamサービスへのアクセスがあった`trace_id`ごとに、同一traceでKeycloakのTOKEN_EXCHANGEイベントが記録されているかを確認する設計だった。

この設計を運用に近い形で見直した結果、2つの問題が判明した。

1. **trace_idはリクエスト単位の相関にしかならない**。委任チェーンの中継トークンを「発行のたびに使い捨てる」実装は現状そうなっている（各APIハンドラが毎回`exchangeForAudience`/`TokenExchangeClient.exchange`を呼ぶ）が、これは実装上の選択であって不変の前提ではない。将来キャッシュして複数リクエストに跨って再利用する実装に変えた場合、2回目以降のリクエストは新たなtrace_idを持つ一方、その中で使われるトークンに対応するTOKEN_EXCHANGEイベントはキャッシュされた1回分しか存在しない。CHECK2は「対応するTOKEN_EXCHANGEが無い」として誤検知する。
2. **trace_idの伝播が途切れると区別できない偽陽性になる**。既存の`docs/audit-demo.md`に記録されている実例（OTelの計装が及ばない経路から手動でToken Exchangeを実行したケース）が示す通り、trace_id相関は「実際にExchangeを経ずに使われた」ケースと「Exchangeは正当に行われたがtrace伝播が途切れただけ」のケースを区別できない。
