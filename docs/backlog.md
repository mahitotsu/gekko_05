...
## UC8（全支店在庫照会・物流管理）のWebUI導線

`docs/use-cases.md`のUC8（`warehouse-viewer-all`ロールでの全支店在庫照会）は認可ロジック自体はwarehouse-serviceに実装済みだが、frontendに対応する画面・APIルートが無いため、edge-proxy経由の実トラフィックでは検証できていない。物流管理向けの画面（支店別在庫照会UI）とそれを呼ぶ`server/api/*`ルートの追加が必要。
